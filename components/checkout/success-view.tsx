...
export function SuccessView({ orderNumber, totalAmount, items }: SuccessViewProps) {
  ...
  useEffect(() => {
    // Track Purchase event with value and currency for Meta Pixel
    if (typeof window.fbq !== 'undefined' && orderNumber && !trackedRef.current) {
      ...
      if (typeof totalAmount === 'number' && totalAmount >= 0) {
        purchaseData.value = totalAmount;
      }

      if (items && items.length > 0) {
        purchaseData.content_type = 'product';
        purchaseData.contents = items;
        purchaseData.content_ids = items.map((item) => item.id);
      }
